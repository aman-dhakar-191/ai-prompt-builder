...
// Fallback models for selection (used when API fetch fails)
 ...
 * @returns {Promise<Array<{id: string, name: string, context_length: number, pricing: object}>>} - Array of available models
 */
...
async function makeOpenRouterRequest(model, messages, apiKey, maxTokens = 2048) {
  ...
  if (!response.ok) {
    ...
    // Provide helpful guidance for data policy errors
...
export async function validateSystemInstructions(systemInstruction, testPrompt, expectedBehavior, apiKey, model = VALIDATOR_MODEL) {
  ...
  const testResponse = await makeOpenRouterRequest(model, testMessages, apiKey, 2048);
